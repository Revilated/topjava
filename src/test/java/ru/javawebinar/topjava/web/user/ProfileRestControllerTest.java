...
class ProfileRestControllerTest extends AbstractControllerTest {
    ...
    void registerNotValid() throws Exception {
        var invalid = new UserTo(null, "newUser", null, null, 1000);
    ...
    void updateNotValid() throws Exception {
        var invalid = new UserTo(USER_ID, user.getName(), null, null, user.getCaloriesPerDay());
        perform(MockMvcRequestBuilders.put(REST_URL).contentType(MediaType.APPLICATION_JSON)
                .with(userHttpBasic(user))
                .content(JsonUtil.writeValue(invalid)))
                .andDo(print())
    ...
    void updateWithDuplicateEmail() throws Exception {
        var updated = new UserTo(USER_ID, user.getName(), admin.getEmail(), user.getPassword(), user.getCaloriesPerDay());
        perform(MockMvcRequestBuilders.put(REST_URL).contentType(MediaType.APPLICATION_JSON)
                .with(userHttpBasic(user))
